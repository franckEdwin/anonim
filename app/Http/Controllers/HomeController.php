...
class HomeController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            // Validez le fichier
            $request->validate([
                'image' => 'image|mimes:jpeg,png,jpg,gif,svg|max:2048', // Ajustez selon vos besoins
            ]);
            // Enregistrez l'image sur le serveur
            $path = $request->file('image')->store('images');
            // Ajoutez le chemin de l'image à la création de l'item
            $create_item->update(['image' => $path]);
        }
